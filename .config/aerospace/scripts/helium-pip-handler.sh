...
#
# The signal that actually works: real Helium browser windows always contain
# " - Helium" in their title (Helium appends " - <Profile>" after it, e.g.
# "New Tab - Helium - You"). PiP windows (and Chromium app/PWA windows) do
# not contain it (e.g. "Meet - abc-defg-hij - You").
#
# Flow:
#   1. Title says "Picture in Picture" outright -> float, leave in place.
#   2. Title contains " - Helium" -> normal window, tile + workspace 1. Instant,
#      no waiting, so ordinary browsing has no visible lag.
#   3. Ambiguous (empty or no marker yet) -> do nothing, poll briefly. If the
#      marker shows up it was just a slow-loading normal window. If it never
...
# Case 2: contains the app-name marker -> ordinary browser window. Act immediately.
...
# Never gained the app marker -> PiP (or a Chromium app window). Float, stay put.
log "no app marker after watch window -> PiP, floating on ws=$origin_workspace"
float_in_place "$origin_workspace"
exit 0
